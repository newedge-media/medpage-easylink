...
class Magik_Lambysettings_Adminhtml_RestoreController extends Mage_Adminhtml_Controller_Action
{
    ...
    {
        $this->loadLayout()
            ->_setActiveMenu('magik/lamby/restore')
            ->_addBreadcrumb(Mage::helper('lambysettings')->__('Restore Defaults'), Mage::helper('lambysettings')->__('Restore Defaults'));
    ...
    public function indexAction()
    {
        ...
        $this->_title($this->__('Magik'))
            ->_title($this->__('Lamby'))
            ->_title($this->__('Restore Defaults'));

        $this->_addContent($this->getLayout()->createBlock('lambysettings/adminhtml_restore_edit'));
    ...
    public function restoreAction()
    {
	    ...
	    try { 
            ...
            $defaults->loadFile(Mage::getBaseDir().'/app/code/local/Magik/Lambysettings/etc/config.xml');
            $this->_restoreSettings($defaults->getNode('default/lambysettings')->children(), 'lambysettings');

		    if ($setup_cms) {
                Mage::getModel('lambysettings/settings')->setupCms();
            }

            Mage::getSingleton('adminhtml/session')->addSuccess(
                Mage::helper('lambysettings')->__('Lamby Theme Settings has been restored. Please clear cache (System > Cache management) if you do not see changes in storefront'));
        }
        catch (Exception $e) {
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('lambysettings')->__('An error occurred while restoring theme settings.'));
        }
